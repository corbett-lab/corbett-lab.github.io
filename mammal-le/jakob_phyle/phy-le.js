...
function getNodeIds(tree) {
  ...
  function traverse(node) {
    ...
    if (isNonEmptyAndNoNumbers(node.name)) { 
      ids.push(node.name);
    }
  }
  traverse(tree);
...
$("#taxonInput").autocomplete({
  ...
  $("#addItemButton").click(function() {
    ...
    if (selectedTaxon == "") {
      //pick five at random
      const possibleHints = getRandomValuesFromArray(taxa);
