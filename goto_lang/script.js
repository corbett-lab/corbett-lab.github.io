...
document.getElementById('right-text').addEventListener('input', function() {
    // Function to be called anytime a character is put into the right text field
    var lastCharacter = this.value[this.value.length - 1];
    console.log('Last character added to the right text field:', lastCharacter);
});
...
document.getElementById('run-button').addEventListener('click', function() {
    ...
    var rightTextFieldValue = document.getElementById('right-text').value;
    console.log('Left Text Field:', leftTextFieldValue);
    console.log('Right Text Field:', rightTextFieldValue);
  ...
  switch (label) {
    ...
    default:
      if (label.startsWith("put_") && label.length === 6) {
        ...
        //console.log(char); //TODO output to 'terminal'
        terminal.value += char
        return O + L + 1;
      }
